...
public class Main {
    ...
    public void solve() throws Exception {
        ...
        int y = 0;
        int n;
        n = (int) Math.pow(2.0, cnt);
        for(int i = 0; i < n; i++) {
            y = 0;
            for(Map.Entry<String, Boolean> entry : map.entrySet()) {
                String s1 = entry.getKey();
                Boolean b = entry.getValue();
                map.put(s1, booleansList.get(y));
                y++;
            }
            if(!exp.checkEntry(map)) {
            ...
            nextBooleansList(booleansList);
        }

        //if isValid:
        for(int i = 0; i < cnt; i++) {
            booleansList.set(i, false);
        }
        for(int i = 0; i < n; i++) {
            y = 0;
            String t = new String();
            ...
            t += s;
            nextBooleansList(booleansList);
            List<String> developmentWithPropositions = new ArrayList<String>();
            developmentWithPropositions.add(t);
            exp.createdDevelopment(map, developmentWithPropositions);
            writeDeductionTheoremWithoutPropositions(developmentWithPropositions);
            developmentWithPropositions.clear();
        }




        //out.print("Внимание, теорема об исключении\n");

        for(int i = 0; i < listForLemmaAboutException.size(); i++) {
            out.print(listForLemmaAboutException.get(i));
            writeEnd();
        }

        lemmaAboutException(listForLemmaAboutException);
    }
